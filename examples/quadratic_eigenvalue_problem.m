...
R = be_residual({A0 + dA0, A1 + dA1, A2 + dA2}, f, V, diag(L));
fprintf('Norm of the residual for the perturbed problem: %e\n', norm(R, 'fro'));

% f = {@(x) 1, @(x) x, @(x) x^2};
D = be_unstructured({A0 + dA0, A1 + dA1, A2 + dA2}, f, V, diag(L));
R = be_residual({A0+dA0+D{1}, A1+dA1+D{2}, A2+dA2+D{3}}, f, V, diag(L));
